// Apply the deterministic impact_sectors floor to the live catalogue.
//
...
import { SECTOR_VOCABULARY, ensureExplicitSectors, stripNonSectorSpans } from '../src/lib/sector-vocabulary'
...
const SOURCE = 'ai_classifier:sector_backstop:v1'
...
function evidence(pattern: RegExp, text: string): string {
  for (const s of stripNonSectorSpans(text).split(/(?<=[.!?;])\s+|\n/)) {
...
async function main() {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await db
      .from('scraped_grants')
      .select('id, funder, title, description, impact_sectors, funder_brief, field_provenance')
      .eq('is_active', true)
      .range(from, from + 999)
  ...
  for (const r of rows) {
    ...
    const before = r.impact_sectors ?? []
    const after = ensureExplicitSectors(before, src)
    let added = after.filter(t => !before.includes(t))
    if (onlyTag) added = added.filter(t => t === onlyTag)
    if (!added.length) continue

    const why = added.map(tag => {
      const entry = SECTOR_VOCABULARY.find(e => e.sector === tag)!
