...
const FIX: { id: string; why: string; fields: Record<string, unknown> }[] = [
  { id: 'dc2eb270-0000', why: 'ECB Building Belonging: projects are identified and applications initiated by county cricket boards, not by clubs; nothing an organisation applies to',
  ...
  { id: '798323fe-0000', why: 'Equity Charitable Trust: page says spending is typically in the region of £5K',
    fields: { amount_max: 5000, amount_undisclosed: false } },
  { id: 'fb2c7933-0000', why: 'Funding Differently: £5k tier for income under £50k, £10k tier for £40k to £150k; closes 23:59 on 21 Oct',
    fields: { max_org_income: 150000 } },
  { id: '95772b99-0000', why: 'Savoy: trustee meeting deadlines 7 Aug, 22 Oct, 21 Jan',
...
async function main() {
  const db = getAdminDb()
  for (const f of FIX) {
    const { data } = await db.from('scraped_grants').select('id,title').like('id', f.id.slice(0, 8) + '%').single()
