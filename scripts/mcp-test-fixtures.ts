...
async function destroy() {
  for (const f of FIXTURES) {
    const userId = await findUserByEmail(f.email)
    if (!userId) { console.log(`absent        ${f.key}`); continue }
    // oauth_tokens and organisations cascade from the user, but delete tokens
    // explicitly first so a credential is never left live even briefly.
    await sb.from('oauth_tokens').delete().eq('user_id', userId)
    await sb.from('organisations').delete().eq('owner_id', userId)
    const { error } = await sb.auth.admin.deleteUser(userId)
    if (error) throw new Error(`deleteUser(${f.email}): ${error.message}`)
    console.log(`destroyed     ${f.key}`)
  }
