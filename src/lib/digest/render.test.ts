...
const model: DigestModel = {
  ...
  closing: [
    { kind: 'pipeline', name: 'Projects for Young People Grants', funder: 'Heathrow Community Trust',
      ...
      url: 'https://www.shootsfunding.co.uk/grants/pfyp', key: 'p1' },
    { kind: 'saved', name: 'A Saved Fund', funder: 'Someone & Co',
      ...
      url: 'https://www.shootsfunding.co.uk/grants/saved-1', key: 's1' },
  ],
  closingOverflow: 0,
  inProgress: [
    { name: 'Church and Communities Programme', funder: null, stage: 'Submitted',
      status: 'In Submitted.', stalled: false, url: 'https://www.shootsfunding.co.uk/grants/ccp', key: 'ip1' },
  ],
  inProgressOverflow: 0,
  matches: [
    { title: 'NCVO Learning & Development', funder: 'NCVO', blurb: 'Training programmes.',
      meta: 'NCVO · rolling', url: 'https://www.shootsfunding.co.uk/grants/ncvo', key: 'm1' },
  ],
  matchesOverflow: 0,
  matchTotal: 1,
  ...
  nearMisses: [
    { title: 'Community Grants Fund', funder: 'X', verdict: 'Ruled out on area.',
      ...
      url: 'https://www.shootsfunding.co.uk/grants/cgf', key: 'nm1' },
  ],
...
describe('every named opportunity is reachable', () => {
  ...
  it.each([
    ['closing pipeline row', 'https://www.shootsfunding.co.uk/grants/pfyp'],
    ['closing saved row',    'https://www.shootsfunding.co.uk/grants/saved-1'],
    ['in-progress row',      'https://www.shootsfunding.co.uk/grants/ccp'],
    ['match row',            'https://www.shootsfunding.co.uk/grants/ncvo'],
    ['near-miss row',        'https://www.shootsfunding.co.uk/grants/cgf'],
  ])('%s links to its page', (_label, href) => {
    expect(html).toContain(`href="${href}"`)
  })

  it('wraps the title text itself in the anchor, not just a button below it', () => {
    expect(html).toMatch(/<a href="[^"]*\/grants\/ncvo"[^>]*>NCVO Learning &amp; Development<\/a>/)
  })
})
