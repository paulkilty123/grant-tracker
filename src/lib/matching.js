 ...
 */
function classifyLocationTag(tag) {
    if (!tag)
    ...
    if (t === 'england')
        return { kind: 'england', label: 'England' };
    if (t === 'scotland')
        return { kind: 'scotland', label: 'Scotland' };
    if (t === 'wales')
        return { kind: 'wales', label: 'Wales' };
    if (t === 'northern ireland' || t === 'ni')
        return { kind: 'ni', label: 'Northern Ireland' };
    if (t === 'international')
        return { kind: 'national', label: 'International' };
 ...
 */
function orgMatchesRegionalTag(tagLabel, orgLocation) {
    const tagLower = tagLabel.toLowerCase().trim();
    const orgLower = orgLocation.toLowerCase();
    if (!tagLower || !orgLower)
        return false;
    // Direct substring match either way
    if (orgLower.includes(tagLower) || tagLower.includes(orgLower.split(',')[0].trim()))
        return true;
 ...
 */
function parseIncomeCapFromText(text) {
    const patterns = [
 ...
 */
function orgIncomeWithinCap(band, cap) {
    if (!band)
        return true;
 ...
 */
function phraseHitRatio(term, text) {
 ...
 */
function normalizeStructureTokens(s) {
 ...
 */
function orgStructuresToCheck(org) {
    if (org.legal_structure)
        return [org.legal_structure];
    switch (org.org_type) {
...
export function computeMatchScore(grant, org, feedback) {
    const reasons = [];
    ...
    if (org.primary_location) {
        ...
        if (tagClass.kind === 'national') {
            ...
            const nationOk = (tagClass.kind === 'england' && orgInEngland) ||
            ...
            if (regionOk) {
                ...
                if (tagClass.label.toLowerCase() === 'london' && (city === 'london' || region.includes('london'))) {
                    ...
                    if (mentionedBoroughs.length > 0) {
                        const orgBoroughMentioned = city !== 'london' && mentionedBoroughs.some(b => b === city || b.includes(city) || city.includes(b));
            ...
            if (grant.isLocal) {
                ...
                const matchedRegion = Object.entries(REGIONAL_KEYWORDS).find(([keyword]) => grantTitleLower.includes(keyword));
                if (matchedRegion) {
    ...
    if (orgImpactSectors.length > 0 && grantImpactSectors.length > 0) {
        ...
        const outcomeTerms = (org.key_outcomes ?? [])
        ...
        if (allOrgTerms.length === 0) {
            ...
            for (const term of orgTerms) {
                const weight = 1.5;
                totalWeight += weight;
                if (fuzzyOverlap(term, grantText))
                    weightedHits += weight;
            }
            ...
            if (ratio >= 0.4)
                ...
                reasons.push('Partial theme match');
        }
    ...
    if (orgHasProfile && !primaryDomainMismatch) {
        ...
        for (const { words, orgTerms } of TITLE_DOMAIN_KEYWORDS) {
            if (words.some(w => titleLower.includes(w))) {
                ...
                const orgCovers = orgTerms.some(t => orgSectorSet.has(t) ||
    ...
    if (!primaryDomainMismatch && orgHasProfile) {
        const funderBrief = grant.funderBrief;
        const wtf = funderBrief && typeof funderBrief === 'object' ? String(funderBrief.what_they_fund || '') : '';
        if (wtf.length > 0) {
            const wtfLower = wtf.toLowerCase();
    ...
    const isCICLike = org.legal_structure === 'cic_guarantee' ||
        org.legal_structure === 'cic_shares' ||
        (org.legal_structure == null && org.org_type === 'cic');
    const isSELike = org.legal_structure === 'ltd_guarantee' ||
    ...
    if (eligibilityText) {
        ...
        if (requiresFaithBuilding) {
            const orgHasFaith = (org.themes ?? []).some((t) => ['faith', 'religion', 'church', 'worship'].some(f => t.includes(f))) ||
        ...
        if (org.primary_location) {
            ...
            const allNations = ['scotland', 'wales', 'northern ireland', 'england'];
            const nationRestrictions = allNations.filter(n => {
                ...
                const inElig = eligibilityText.includes(`based in ${n}`) ||
            ...
            if (nationRestrictions.length > 0) {
                const orgMatchesNation = nationRestrictions.some(n => (n === 'scotland' && isInScotland) ||
                    (n === 'wales' && isInWales) ||
            ...
            if (orgRegion.includes('london') || city === 'london') {
                const mentionedBoroughs = LONDON_BOROUGHS.filter(b => grantText.includes(b));
                if (mentionedBoroughs.length > 0) {
                    const orgBoroughMentioned = mentionedBoroughs.some(b => b === city || b.includes(city) || city.includes(b));
                    if (!orgBoroughMentioned) {
        ...
        if (org.years_operating != null && eligibilityText.length > 20) {
            const accountsMatch = eligibilityText.match(/(?:minimum\s+)?(\d+)\s+(?:full\s+)?years?\s+(?:of\s+)?(?:published\s+)?accounts|(\d+)\s+years?\s+(?:trading|operating|established)/i);
            if (accountsMatch) {
    ...
    let beneficiaryMismatch = false;
    {
        ...
        const oBen = org.beneficiary_groups ?? [];
        const grantIsGeneral = gBen.length === 0 || (gBen.length === 1 && gBen[0] === 'general_public');
        ...
        if (grantIsGeneral || orgIsGeneral) {
            ...
            const gSet = new Set(gBen.filter(b => b !== 'general_public'));
            const oSet = new Set(oBen);
            ...
            if (gSet.size === 0) {
                ...
                // No overlap at all — the grant targets specific groups the org doesn't serve
                beneficiaryScore = 1;
                beneficiaryMismatch = true;
                reasons.push('Grant targets different beneficiary groups — check fit');
            }
        }
    }
    // ── Total ──────────────────────────────────────────────────────────────
    let score = Math.min(100, locationScore + themesScore + grantSizeScore + funderTypeScore + eligibilityScore + beneficiaryScore);
...
export function scoreColour(score) {
    if (score >= 80)
        return { bg: 'bg-sage/15', text: 'text-sage', bar: 'bg-sage' };
    if (score >= 65)
        return { bg: 'bg-gold/15', text: 'text-gold', bar: 'bg-gold' };
    if (score >= 45)
        return { bg: 'bg-warm', text: 'text-mid', bar: 'bg-mid' };
