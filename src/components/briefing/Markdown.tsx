...
const COMPONENTS: Components = {
  ...
  a: ({ children, href }) => (
    <a href={href} target="_blank" rel="noreferrer" className="underline" style={{ color: 'var(--state-success)' }}>{children}</a>
  ),
  ...
  blockquote: ({ children }) => (
    <blockquote className="pl-3 my-2" style={{ borderLeft: '3px solid var(--border-warm)', color: 'var(--text-muted)' }}>{children}</blockquote>
  ),
  hr: () => <hr className="my-2" style={{ border: 0, borderTop: '1px solid var(--border-warm)' }} />,
  code: ({ children }) => (
    <code className="px-1 py-0.5 text-[12px]" style={{ background: 'var(--surface-sunken)', borderRadius: 6, fontFamily: 'ui-monospace, SFMono-Regular, monospace' }}>{children}</code>
  ),
  pre: ({ children }) => (
    <pre className="overflow-x-auto my-2 p-2.5 text-[12px]" style={{ background: 'var(--surface-sunken)', borderRadius: 8 }}>{children}</pre>
  ),
  table: ({ children }) => (
  ...
  th: ({ children }) => (
    <th className="text-left px-2 py-1 font-semibold" style={{ ...grotesk, background: 'var(--surface-sunken)', border: '1px solid var(--border-warm)', color: 'var(--text-body)' }}>{children}</th>
  ),
  td: ({ children }) => (
    <td className="px-2 py-1 align-top" style={{ border: '1px solid var(--border-warm)' }}>{children}</td>
  ),
}
