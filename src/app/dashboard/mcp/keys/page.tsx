// MCP API key management UI — list user's keys with revoke action.
...
export default async function MCPKeysPage() {
  ...
  if (!user) redirect('/auth/login?next=/dashboard/mcp/keys')
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-mid">
            Keys for connecting AI agents (Claude, ChatGPT, Gemini) to the Grant Tracker MCP server.
          </p>
        </div>
        <Link
          href="/dashboard/mcp/keys/new"
      ...
      {keys.length === 0 ? (
        <div className="rounded-xl border border-warm p-10 text-center" style={{ background: '#FAFAF7' }}>
