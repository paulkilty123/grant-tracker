...
export default function UrlAdminPage() {
  ...
  function computeBriefUpdates(grant: Grant): Record<string, string | boolean | number | null> {
    ...
    // ── Extract amounts ───────────────────────────────────────────────────────
    // The cue-based pool-vs-per-grant logic that used to live inline here now
    // lives in src/lib/grant-amounts.ts, so the AUTOMATED enrich chain uses the
    // same implementation instead of a cruder regex. Behaviour here is
    // unchanged: the shared function derives the values, and the pending-edit
    // guard below is still applied by this component.
    // Promoted 2026-07-25 — see the header of grant-amounts.ts for why.
    const { amount_min: derivedMin, amount_max: derivedMax } = extractGrantAmounts(awardText)
    if (derivedMax !== null) {
      if (!getReviewVal(grant.id, 'amount_max', null)) updates.amount_max = derivedMax
      if (derivedMin !== null && !getReviewVal(grant.id, 'amount_min', null)) {
        updates.amount_min = derivedMin
      }
    }
