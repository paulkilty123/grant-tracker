...
export default function UrlAdminPage() {
  ...
  function populateFromBrief(grant: Grant) {
    ...
    const POOL_CUES_LEFT = /\b(?:awarded?\s+(?:a\s+)?total|totalling|total\s+(?:of|awarded|distributed|grants?|funding|funds|fund)\b|in\s+(?:the\s+)?(?:past|previous|last)\s+(?:year|years|few\s+years)|annual(?:ly\s+(?:awards?|distributes?|gives?|spends?)|\s+(?:budget|fund|spending|expenditure))|per\s+(?:year|annum)|each\s+year|distributes?|donates?|spends?|gives?\s+(?:away|out)|endowment|combined\s+(?:total|funding|budget)|annual\s+(?:income|turnover)\s+of|income\s+(?:of|cap|limit|under|below|up\s+to)|turnover\s+of|reserves\s+of)\b/i
    const POOL_CUES_RIGHT = /^[\s,()]*(?:distributed\s+across|spread\s+across|split\s+(?:across|between|among)|shared\s+(?:across|between|among)|across\s+(?:multiple|several|all)|to\s+multiple|in\s+20\d{2}(?:[\/\-]\d{2,4})?\b|in\s+total\b|altogether|donated\s+across|across\s+(?:charities|recipients|organisations|projects)|or\s+(?:less|below|under)\b|or\s+fewer\b)/i
