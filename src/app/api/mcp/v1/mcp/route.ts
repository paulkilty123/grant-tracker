...
// let compatible clients (e.g. the Connectors Directory) render the brand
// mark from metadata instead of guessing at the favicon.
...
const MCP_SERVER_INFO = {
  name: brand.mcp.serverSlug,
  version: '1.4.0',
  websiteUrl: brand.siteUrl,
  icons: [
    { src: `${brand.siteUrl}/favicon.svg`, mimeType: 'image/svg+xml', sizes: ['any'] },
    { src: `${brand.siteUrl}/android-chrome-512x512.png`, mimeType: 'image/png', sizes: ['512x512'] },
  ],
}
...
async function runCompanion(
  ...
  if (!auth?.orgId || auth.tier !== 'companion') {
    return companionError('forbidden', `Adviser tools require an Adviser-tier ${brand.name} account linked to this connection.`)
  }
  ...
  try {
    ...
    return companionError('internal_error', `Something went wrong. Please retry; if it persists, contact ${brand.email.hello}.`)
  }
}
        ...
        try {
          ...
          return {
            content: [{ type: 'text', text: JSON.stringify({
              error: { code: 'internal_error', message: `Search failed. Please retry; if this persists, contact ${brand.email.hello}.` },
              attribution: ATTRIBUTION,
              rate_limit_status: rateLimitStatusForContext(auth),
            }) }],
            isError: true,
    ...
    server.tool(
      ...
      async ({ opportunity_id, include_funder_summary }) => {
        ...
        if (error) {
          ...
          return {
            content: [{ type: 'text', text: JSON.stringify({
              error: { code: 'internal_error', message: `Could not load this opportunity. Please retry; if this persists, contact ${brand.email.hello}.` },
              attribution: ATTRIBUTION,
              rate_limit_status: rateLimitStatusForContext(auth),
            }) }],
            isError: true,
...
const WWW_AUTHENTICATE_VALUE =
  `Bearer realm="${brand.mcp.serverSlug}", ` +
  `resource_metadata="${brand.siteUrl}/.well-known/oauth-protected-resource"`
...
function unauthorisedResponse(reason: 'invalid_token' | 'revoked_token' | 'no_credentials'): NextResponse {
  const message = (() => {
    ...
    return `Authorization required. MCP clients should follow the OAuth flow advertised in WWW-Authenticate; developers can use a ${brand.mcp.productName} key from ${brand.domain}/mcp.`
  })()
