...
3. **Briefing-first surface** — NOT BUILT YET, and **BLOCKED (8 Jul 2026) on the incoming design spec package** (briefing page, goal-setup flow, Plan page). Do not build the page or thread persistence until it lands. Known schema implications to expect: a `purposes` structure on goals (purpose split drives a recommended mix); mix vocabulary standardising on funding character (unrestricted/project/capital) rather than source; per-stage likelihood weights for the weighted-pipeline figure (replacing the hardcoded STAGE_WEIGHT map in context.ts); one new contract constant (no repayable-finance advice — landscape and signposting only).
...
4. **Eval harness, conversational cases** — NOT BUILT YET: extend the harness with multi-turn cases seeded from the MCP test transcript — the draft-refusal, the unverified-row handling, the mix inference. (The live harness's T2/T5 smoke-test mix inference and draft-refusal informally; T4's secured-amount confabulation, 14.3.8, is the seed for the unverified-claim case.)
...
8. **NEW (8 Jul, exposed by T4): outcomes don't move the plan arithmetic.** `goals.secured_amount` is a snapshot taken at set_funding_goal time; a later won-stage transition changes the *weighted pipeline* figure but not secured/gap. §5.1 specified secured as "derived-but-cached: recomputed from pipeline won-stage amounts on read" — the read path (`getGoal`) doesn't recompute. The complication is representing off-pipeline secured (the explicit `secured_amount` override) so a read-side recompute doesn't erase it — that's goal-schema surgery, so **fold the fix into the design-spec package** (which already touches goals). Until then the strategist's gap goes stale after wins. Just as important: the model papered over the anomaly with a confident, wrong explanation ("the £30,000 was already reflected in your secured figure") — the canonical unverified-claim eval case for 14.1.4.
