...
3. **Briefing-first surface** — design spec LANDED 9 Jul 2026 (Drive: `companion-v1-design-spec` — the canonical text for the briefing page, goal setup, and plan page; build order in its §9). **Steps 1–2 of that order are BUILT (10 Jul):**
   ...
   **Ship gates with Paul:** the mix rulebook review + stage-weights table — review pack at `docs/goal-agent/mix-rulebook-review.md` (one markable row per rule, fallback behaviour, blending mechanics, weights). Gates the recommendation going live, not the build.
