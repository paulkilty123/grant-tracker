...
3. **Briefing-first surface** — design spec LANDED 9 Jul 2026 (Drive: `companion-v1-design-spec` — the canonical text for the briefing page, goal setup, and plan page; build order in its §9). **Steps 1–2 of that order are BUILT (10 Jul):**
   ...
   - *Remaining (spec §9 order):* (3) thread persistence → (4) briefing page → (5) setup flow → (6) plan page. **Requirement recorded for (4), from Paul 9 Jul:** the `companion_access` flag governs NAVIGATION as well as routes — a non-flagged user sees no Briefing/Plan items and no trace the surface exists; a flagged user's Dashboard item becomes Briefing; the flag is the single source of truth for API and UI visibility, no separate UI toggle that can drift.
   ...
   **Small follow-on (flagged 10 Jul):** the spec's adjustment example quotes the new run-rate *before* writing ("that lifts the run-rate to £19.4k/month"); pre-write figures need a what-if/preview capability the tools don't have. V1 behaviour: confirm intent in words, write, re-fetch, report the new figures verbatim.
