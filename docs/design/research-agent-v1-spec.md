...
6. The eval set → ship to Paul's orgs behind the existing gates. — EVALS DONE, verified live 2026-07-13 (`scripts/agent-eval/research-conversation-eval.ts`, 2 consecutive clean runs, 15/15 checks). All four spec §6 cases pass: provenance grounding, unverified-find restricted actions, discrepancy honesty, over-budget honesty (the allowed=false branch, forced via seeded events rather than an env-var hack). Scaffold guard extended to the brief artefact as a pure case in tools-smoke.ts (`lintApplicationVoice`), not a live eval, per the spec's own framing. Two real bugs found by running the evals, both fixed: (1) Anthropic's server-tool container id now threads across a turn's iterations (loop.ts); (2) the restricted-actions rule is now structural, not prompt-only — loop.ts refuses add_to_pipeline in a research turn with no catalogue opportunity_id, before dispatchTool runs, because the steering alone didn't hold under a direct "add it anyway" push. **Ship gate now Paul's full walkthrough on the preview** — merge to main only once both the evals (done) and his review (in progress) pass.
