...
- Beneficiary groups taxonomy: working hypothesis, derived from DB frequency analysis of `target_beneficiaries` + `beneficiary_tags`, pending manual curation. To be filled in before `get_taxonomy` is implemented (step 5). See appendix for raw frequencies.
